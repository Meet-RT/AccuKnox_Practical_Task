...
class UserRegisterView(APIView):
    ...
    def post(self, request):
        if User.objects.filter(email=request.data['email']).exists():
            return Response({'error': 'Email already registered in system'}, status=status.HTTP_400_BAD_REQUEST)
        else:
...
class UserListViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        if '@' in keyword:  # If the parameter contains '@', assume it's an email search
...
class FriendRequestViewSet(viewsets.ModelViewSet):
    ...
    def friends(self, request):
        friends = FriendRequest.objects.filter((models.Q(from_user=request.user) | models.Q(to_user=request.user)),is_accepted=True,is_rejected=False
    ...
    def send_friend_request(self, request):
        ...
        check_requests = FriendRequest.objects.filter(from_user=request.user,created_at__gte=timezone.now() - timezone.timedelta(minutes=1)).count()

        if check_requests >= 3:
            return Response({'detail': 'You have reached the limit of 3 requests within a minute.'},status=status.HTTP_429_TOO_MANY_REQUESTS)
